...
//         self.bids.iter().map(|x| x.qty).sum()
//     }
//
//     fn ask_volume(&self) -> u32 {
//         self.asks.iter().map(|x| x.qty).sum()
//     }
//
...
//
//         orderbook.match_orders(10, &mut executions_buffer); // Then
...
//         orderbook.add_order(buy_order);
//         orderbook.add_order(sell_order);
//         orderbook.add_order(latter_sell_order);
//         // When
//         let mut executions_buffer: [Execution; 10] = uninitialized_arr();
//         orderbook.match_orders(10, &mut executions_buffer);
//         // Then
//         assert!(orderbook.bids.is_empty());
