...
fn multicast_receiver_to_engine_msg_in(mut udp_socket: &UdpSocket, mut oe_tx: &Sender<Order>) -> ! {
    ...
    loop {
        ...
        let inbound_engine_message: SequencedEngineMessage = bitcode::decode(&buffer[..size]).unwrap();

        let ack_bits = inbound_engine_message.sequence_number.to_le_bytes();
        udp_socket.send_to(&ack_bits, addr).expect("TODO: panic message");
